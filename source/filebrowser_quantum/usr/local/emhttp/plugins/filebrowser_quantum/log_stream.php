...
if (!file_exists($logfile)) {
    echo "data: [Error] 无法找到日志文件: " . htmlspecialchars($logfile) . " \n\n";
    flush();
    exit;
}
...
// 调试点 2: 显式告知连接成功（XSS 防护：转义所有动态内容）
$safe_logname = htmlspecialchars(basename($logfile), ENT_QUOTES, 'UTF-8');
$safe_level = htmlspecialchars(strtoupper($level), ENT_QUOTES, 'UTF-8');
echo "data: [System] 已连接到日志: $safe_logname (Level: $safe_level)\n\n";
flush();
